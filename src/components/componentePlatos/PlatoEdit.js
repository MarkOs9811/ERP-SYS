...
export function PlatoEditar({dataPlato,handleCloseModal}){
    ...
    const [selectedCategoria, setSelectedCategoria] = useState(dataPlato.idCategoria || '');
    ...
    const onSubmit = async (data) => {
        try {
            ...
            console.log('ID del plato:', dataPlato.id);
            ...
            const response = await axiosInstance.post(`/gestionPlatos/updatePlato/${dataPlato.id}`, 
                formData
            );
            if (response.data.success) {
            ...
            if (error.response && error.response.data) {
                ToastAlert('error', error.response.data.message || 'Error de validación');
            } else {
        ...
        <div>
            <form onSubmit={handleSubmit(onSubmit)} className="px-2">
                <div className="card d-flex">
                    <label htmlFor="foto" className="form-label">Foto Plato</label>
                    <div className="mb-4">
                        <div {...getRootProps()} className="dropzone border rounded p-4 text-center" style={{ cursor: 'pointer' }}>
                        <img src={fotoPreview || `${BASE_URL}/storage/${dataPlato.foto}`} width={150} className="rounded m-auto"/>
                            <input {...getInputProps()} id="foto" name="foto" className="d-none" />
                            {!fotoPreview && (
