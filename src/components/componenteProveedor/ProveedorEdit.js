...
import { faEnvelope, faIdCard, faMapMarkedAlt, faPhone, faUser } from "@fortawesome/free-solid-svg-icons";
import axiosInstance from "../../api/AxiosInstance";
import { handleInputChange, handleSelectChange,limitTelefonoInput, validateTelefono  } from "../../hooks/InputHandlers";

export function ProveedorEdit({ dataProveedor, handleProveedorUpdated, handleCloseModal }) {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await axiosInstance.put(`/proveedores/updateProveedores/${dataProveedor.id}`, data);
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="col-4">
            <div className="form-floating mb-3">
              <select
                ...
                {...register("tipo_documento", { required: "Este campo es obligatorio" })}
                value={tipoDocumento}
              ...
              {errors.tipo_documento && <div className="invalid-feedback">{errors.tipo_documento.message}</div>}
            </div>
          </div>
          <div className="col-4">
            <div className="form-floating mb-3">
              ...
              {errors.numero_documento && <div className="invalid-feedback">{errors.numero_documento.message}</div>}
            </div>
          </div>
          <div className="col-4">
            <div className="form-floating mb-3">
        ...
        <div className="form-floating mb-3">
          <input
            ...
            {...register("correo", {
              required: "Este campo es obligatorio",
              pattern: { value: /^[^@ ]+@[^@ ]+\.[^@ .]{2,}$/, message: "Correo inválido" },
            })}
          />
          <label>
        ...
        <div className="d-flex justify-content-end">
          <button type="button" className="btn-cerrar-modal me-2" onClick={handleCloseModal}>
            Cancelar
          </button>
